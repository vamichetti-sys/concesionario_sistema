...
def _generar_pdf_lote_pagares_3_por_hoja(pagares):
    ...
    def dibujar(c, pagare, x0, x1):
        ...
        c.setFont("Helvetica-Bold", 16); c.setFillColor(AZUL); c.drawString(x, y, "PAGARÉ")
        ...
        c.drawString(x, y, f"Nº {pagare.numero}  ·  Emitido: {pagare.fecha_emision.strftime('%d/%m/%Y')}  ·  Vence: {venc_str}"); y -= 0.6*cm
        ...
        texto = (f"Debo/Debemos y pagaré/pagaremos mancomunada y solidariamente SIN PROTESTO "
            f"(Art. 50 D. Ley 5965/63), a la orden de {pagare.beneficiario.upper()}, "
            f"la suma de PESOS {pagare.monto:,.0f}, en {pagare.lugar_emision} "
            f"el día {venc_letras}. En caso de mora el deudor pagará intereses punitorios. "
        ...
        c.drawString(x+w*0.55, firma_y-0.32*cm, "Aclaración")
